...
sumLast2 n = last + beforeLast
  where
    last = (abs n) `mod` 10
    beforeLast = ((abs n) `mod` 100) `div` 10
...
-}
firstDigit :: Int -> Int
firstDigit n = firstDigitPositive (abs n)
  where firstDigitPositive x = if x > 9 then firstDigitPositive (x `div` 10) else x
